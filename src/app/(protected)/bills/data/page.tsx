...
export default function BillsDataPage() {
  ...
  const { data, isLoading, error, refetch } = useDataBills({
    ...filters,
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Plan',
        cell: ({ row }) => <span className="text-slate-600 truncate max-w-[200px] block" title={row.getValue('planName')}>{row.getValue('planName')}</span>,
      },
      {
    ...
    <div className="space-y-6">
      ...
      {error && (
        <Alert variant="destructive" className="mb-6">
          <AlertDescription>Failed to load data transactions. Please try again.</AlertDescription>
        </Alert>
      )}
